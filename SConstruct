...
def GetEnvironment():
  ...
  if not env['program'] in ['viem','graphchecker','evaluator','generate_model']:
...
if env['variant'] == 'optimized':
  ...
  # A little bit more output on the console
  env.Append(CXXFLAGS = ' -DNDEBUG -funroll-loops -Wall -g -pg -fno-stack-limit -O3 -std=c++0x')
  env.Append(CCFLAGS  = '-O3  -DNDEBUG -DKAFFPAOUTPUT -g -pg -std=c++0x')
else:
  env.Append(CXXFLAGS = ' -DNDEBUG -Wall -funroll-loops  -fno-stack-limit -O3 -std=c++0x')
  env.Append(CCFLAGS  = '-O3  -DNDEBUG -funroll-loops -std=c++0x ')
  if SYSTEM != 'Darwin':
