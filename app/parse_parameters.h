...
int parse_parameters(int argn, char **argv,
  ...
  struct arg_lit
	  *edge_rating_tiebreaking = arg_lit0(NULL, "edge_rating_tiebreaking", "Enable random edgerating tiebreaking.");
  struct arg_lit *match_islands = arg_lit0(NULL, "match_islands", "Enable matching of islands during gpa algorithm.");
  struct arg_lit *only_first_level = arg_lit0(NULL,
											  "only_first_level",
											  "Disable Multilevel Approach. Only perform on the first level. (Currently only initial partitioning).");
  ...
  struct arg_lit
	  *disable_qgraph_refinement = arg_lit0(NULL, "disable_qgraph_refinement", "Disables qgraph refinement.");
  struct arg_lit
	  *use_fullmultigrid = arg_lit0(NULL, "use_fullmultigrid", "Enable full multigrid (wcycles have to be enabled).");
  ...
  struct arg_lit *first_level_random_matching =
	  arg_lit0(NULL, "first_level_random_matching", "The first level will be matched randomly.");
  struct arg_lit *rate_first_level_inner_outer =
	  arg_lit0(NULL, "rate_first_level_inner_outer", "The edge rating for the first level is inner outer.");
  struct arg_lit
	  *use_bucket_queues = arg_lit0(NULL, "use_bucket_queues", "Use bucket priority queues during refinement.");
  struct arg_lit *use_wcycles = arg_lit0(NULL, "use_wcycle", "Enables wcycles.");
  struct arg_lit *disable_refined_bubbling = arg_lit0(NULL,
													  "disable_refined_bubbling",
													  "Disables refinement during initial partitioning using bubbling (Default: enabled).");
  struct arg_lit *enable_convergence = arg_lit0(NULL,
												"enable_convergence",
												"Enables convergence mode, i.e. every step is running until no change.(Default: disabled).");
  struct arg_lit *enable_omp = arg_lit0(NULL, "enable_omp", "Enable parallel omp.");
  struct arg_lit *wcycle_no_new_initial_partitioning = arg_lit0(NULL,
																"wcycle_no_new_initial_partitioning",
																"Using this option, the graph is initially partitioned only the first time we are at the deepest level.");
  struct arg_str *filename = arg_strn(NULL, NULL, "FILE", 1, 1, "Path to file (graph/model).");
  struct arg_str *filename_output = arg_str0(NULL, "output_filename", NULL, "Specify the name of the output file.");
  struct arg_str *mapping_filename = arg_str0(NULL,
											  "mapping_filename",
											  NULL,
											  "Specify the name of the mapping file, where the partition ID is specified for each vertex. If omitted no mapping file is written");
  struct arg_int *user_seed = arg_int0(NULL, "seed", NULL, "Seed to use for the PRNG.");
  struct arg_int *k = arg_int1(NULL, "k", NULL, "Number of blocks to partition the graph.");
  struct arg_rex *edge_rating = arg_rex0(NULL,
										 "edge_rating",
										 "^(weight|expansionstar|expansionstar2|expansionstar2deg|punch|expansionstar2algdist|expansionstar2algdist2|algdist|algdist2)$",
										 "RATING",
										 REG_EXTENDED,
										 "Edge rating to use. One of {weight, expansionstar, expansionstar2, punch," " expansionstar2deg}. Default: weight");
  struct arg_rex *refinement_type = arg_rex0(NULL,
											 "refinement_type",
											 "^(fm|fm_flow|flow)$",
											 "TYPE",
											 REG_EXTENDED,
											 "Refinementvariant to use. One of {fm, fm_flow, flow}. Default: fm");
  struct arg_rex *matching_type = arg_rex0(NULL,
										   "matching",
										   "^(random|hem|shem|regions|gpa|randomgpa|localmax)$",
										   "TYPE",
										   REG_EXTENDED,
										   "Type of matchings to use during coarsening. One of {random, hem," " shem, regions, gpa, randomgpa, localmax}.");
  ...
  struct arg_lit *mh_penalty_for_unconnected = arg_lit0(NULL,
														"mh_penalty_for_unconnected",
														"Add a penalty on the objective function if the computed partition contains blocks that are not connected.");
  ...
  struct arg_lit *mh_diversify_best =
	  arg_lit0(NULL, "mh_diversify_best", "Uses best individuum instead of random during diversification.");
  struct arg_lit *mh_enable_tournament_selection = arg_lit0(NULL,
  ...
  struct arg_lit *mh_optimize_communication_volume = arg_lit0(NULL,
															  "mh_optimize_communication_volume",
															  "Fitness function is modified to optimize communication volume instead of the number of cut edges.");
  struct arg_lit *disable_balance_singletons = arg_lit0(NULL, "disable_balance_singletons", "");
  struct arg_lit *gpa_grow_internal = arg_lit0(NULL,
											   "gpa_grow_internal",
											   "If the graph is allready partitions the paths are grown only block internally.");
  struct arg_int *initial_partitioning_repetitions = arg_int0(NULL,
  ...
  struct arg_int *aggressive_random_levels = arg_int0(NULL,
													  "aggressive_random_levels",
													  NULL,
													  "In case matching is randomgpa, this is the number of levels that should be matched using random matching. Default: 3.");
  struct arg_dbl *imbalance = arg_dbl0(NULL, "imbalance", NULL, "Desired balance. Default: 3 (%).");
  struct arg_rex *initial_partition = arg_rex0(NULL,
											   "initial_partitioner",
											   "^(metis|scotch|hybrid|bubbling|squeez|metaheuristic|recursive)$",
											   "PARTITIONER",
											   REG_EXTENDED,
											   "Type of matchings to use during coarsening. One of {metis, scotch, bubbling, hybrid, recursive).");
  struct arg_lit *initial_partition_optimize =
	  arg_lit0(NULL, "initial_partition_optimize", "Enables postoptimization of initial partition.");
  struct arg_rex *bipartition_algorithm = arg_rex0(NULL,
												   "bipartition_algorithm",
												   "^(bfs|fm|squeezing)$",
												   "TYPE",
												   REG_EXTENDED,
												   "Type of bipartition algorithm to use in case of recursive partitioning. One of " " {bfs, fm, squeezing}.");
  struct arg_rex *permutation_quality = arg_rex0(NULL,
												 "permutation_quality",
												 "^(none|fast|good|cacheefficient)$",
												 "QUALITY",
												 REG_EXTENDED,
												 "The quality of permutations to use. One of {none, fast," " good, cacheefficient}.");
  struct arg_rex *permutation_during_refinement = arg_rex0(NULL,
														   "permutation_during_refinement",
														   "^(none|fast|good)$",
														   "QUALITY",
														   REG_EXTENDED,
														   "The quality of permutations to use during 2way fm refinement. One of {none, fast," " good}.");
  struct arg_int *fm_search_limit =
	  arg_int0(NULL, "fm_search_limit", NULL, "Search limit for 2way fm local search: Default 1 (%).");
  struct arg_int *bipartition_post_fm_limit = arg_int0(NULL,
													   "bipartition_post_fm_limit",
													   NULL,
													   "Search limit for the fm search after a bipartition has been created. :");
  struct arg_int *bipartition_post_ml_limit = arg_int0(NULL,
													   "bipartition_post_ml_limit",
													   NULL,
													   "Search limit for the multilevel fm search after a bipartition has been created. :");
  struct arg_int *bipartition_tries = arg_int0(NULL,
											   "bipartition_tries",
											   NULL,
											   "Number of tries to find a bipartition (during recursive intial partitioning).");
  struct arg_rex *refinement_scheduling_algorithm = arg_rex0(NULL,
															 "refinement_scheduling_algorithm",
															 "^(fast|active_blocks|active_blocks_kway)$",
															 "QUALITY",
															 REG_EXTENDED,
															 " One of {fast, active_blocks, active_blocks_kway}.");
  struct arg_dbl *bank_account_factor =
	  arg_dbl0(NULL, "bank_account_factor", NULL, "The bank account factor for the scheduler. Default 1.5 (%).");
  struct arg_dbl *flow_region_factor = arg_dbl0(NULL,
												"flow_region_factor",
												NULL,
												"If using flow, then the regions found are sized flow_region_factor * imbalance. Default: 4 (%).");
  struct arg_dbl *kway_adaptive_limits_alpha = arg_dbl0(NULL,
														"kway_adaptive_limits_alpha",
														NULL,
														"This is the factor alpha used for the adaptive stopping criteria. Default: 1.0");
  struct arg_rex *stop_rule = arg_rex0(NULL,
									   "stop_rule",
									   "^(simple|multiplek|strong)$",
									   "VARIANT",
									   REG_EXTENDED,
									   "Stop rule to use. One of {simple, multiplek, strong}. Default: simple");
  struct arg_int *num_vert_stop_factor =
	  arg_int0(NULL, "num_vert_stop_factor", NULL, "x*k (for multiple_k stop rule). Default 20.");
  struct arg_rex *kway_search_stop_rule = arg_rex0(NULL,
												   "kway_stop_rule",
												   "^(simple|adaptive)$",
												   "VARIANT",
												   REG_EXTENDED,
  ...
  struct arg_int *bubbling_iterations =
	  arg_int0(NULL, "bubbling_iterations", NULL, "Number of bubbling iterations to perform: Default 1 .");
  struct arg_int
	  *kway_rounds = arg_int0(NULL, "kway_rounds", NULL, "Number of kway refinement rounds to perform: Default 1 .");
  struct arg_int *kway_fm_limits =
	  arg_int0(NULL, "kway_fm_search_limit", NULL, "Search limit for kway fm local search: Default 1 .");
  struct arg_int *global_cycle_iterations =
	  arg_int0(NULL, "global_cycle_iterations", NULL, "Number of V-cycle iterations: Default 2.");
  struct arg_int *level_split = arg_int0(NULL,
										 "level_split",
										 NULL,
										 "Number of trial tree levels (1 means on each level a two trials are performed). Default: 2.");
  struct arg_int *toposort_iterations =
	  arg_int0(NULL, "toposort_iterations", NULL, "Number of topo sort iterations). Default: 4.");
  struct arg_lit *most_balanced_flows = arg_lit0(NULL, "most_balanced_flows", "(Default: disabled)");
  struct arg_str *input_partition = arg_str1(NULL, "input_mapping", NULL, "Input mapping to use.");
  struct arg_lit *recursive_bipartitioning =
	  arg_lit0(NULL, "recursive_bipartitioning", "Use recursive bipartitioning instead of kway methods.");
  struct arg_lit *suppress_output = arg_lit0(NULL, "suppress_output", "(Default: output enabled)");
  struct arg_lit *disable_max_vertex_weight_constraint = arg_lit0(NULL,
																  "disable_max_vertex_weight_constraint",
																  "Disables the max vertex weight constraint during the contraction.");
  struct arg_int *local_multitry_fm_alpha =
	  arg_int0(NULL, "local_multitry_fm_alpha", NULL, "Search limit factor alpha for multitry fm.");
  struct arg_int
	  *local_multitry_rounds = arg_int0(NULL, "local_multitry_rounds", NULL, "Number of rounds for local multitry fm.");
  struct arg_int *initial_partition_optimize_fm_limits = arg_int0(NULL,
																  "initial_partition_optimize_fm_limits",
																  NULL,
																  "Initial Partition Optimize FM limits. (Default: 20)");
  struct arg_int *initial_partition_optimize_multitry_fm_alpha = arg_int0(NULL,
																		  "initial_partition_optimize_multitry_fm_limits",
																		  NULL,
																		  "Initial Partition Optimize Multitry FM limits. (Default: 20)");
  struct arg_int *initial_partition_optimize_multitry_rounds =
	  arg_int0(NULL, "initial_partition_optimize_multitry_rounds", NULL, "(Default: 100)");
...
#else
  struct arg_rex *preconfiguration = arg_rex0(NULL,
											  "preconfiguration",
											  "^(strong|eco|fast|fastsocial|ecosocial|strongsocial)$",
											  "VARIANT",
											  REG_EXTENDED,
											  "Use a preconfiguration. (Default: strong) [strong|eco|fast|fastsocial|ecosocial|strongsocial].");
#endif

  struct arg_rex *preconfiguration_mapping = arg_rex0(NULL,
													  "preconfiguration_mapping",
													  "^(strong|eco|fast)$",
													  "VARIANT",
													  REG_EXTENDED,
													  "Use a preconfiguration for the partitioning tool within the mapping algorithm. (Default: eco) [strong|eco|fast].");
  ...
  struct arg_int *local_partitioning_repetitions =
	  arg_int0(NULL, "local_partitioning_repetitions", NULL, "Number of local repetitions.");
  struct arg_int *amg_iterations = arg_int0(NULL, "amg_iterations", NULL, "Number of amg iterations.");
  struct arg_int *mh_flip_coin = arg_int0(NULL,
										  "mh_flip_coin",
										  NULL,
										  "Control the ratio of mutation and crossovers. c/10 Mutation and (10-c)/10 crossovers.");
  struct arg_int *mh_initial_population_fraction = arg_int0(NULL,
  ...
  struct arg_lit *mh_sequential_mode =
	  arg_lit0(NULL, "mh_sequential_mode", "Disables all MH algorithms. Use KaFFPa in a parallel setting.");
  struct arg_rex *kaba_neg_cycle_algorithm = arg_rex0(NULL,
													  "kaba_neg_cycle_algorithm",
													  "^(ultramodel|randomcycle|playfield|ultramodelplus)$",
													  "VARIANT",
													  REG_EXTENDED,
													  "Balanced refinement operator to use. On of randomcycle, ultramodel, playfield, ultramodelplus");
  struct arg_dbl *kabaE_internal_bal = arg_dbl0(NULL,
												"kabaE_internal_bal",
												NULL,
												"Control the internal balance paramter for kaffpaE (Default: 0.01) (1 percent)");
  struct arg_int *kaba_internal_no_aug_steps_aug = arg_int0(NULL,
															"kaba_internal_no_aug_steps_aug",
															NULL,
															"Internal number of steps in the augmented models of negative cycle detection.");
  struct arg_int
	  *kaba_packing_iterations = arg_int0(NULL, "kaba_packing_iterations", NULL, "Number of packing iterations.");
  struct arg_int *kaba_unsucc_iterations = arg_int0(NULL,
													"kaba_unsucc_iterations",
													NULL,
													"Number of unsucc iterations until a rebalancing step is performed.");
  struct arg_lit *kaba_flip_packings =
	  arg_lit0(NULL, "kaba_flip_packings", "Enable flip packing mode (if ultramodelplus is used).");
  struct arg_rex *kaba_lsearch_p = arg_rex0(NULL,
											"kaba_lsearch_p",
											"^(coindiff|coinrnd|nocoindiff|nocoinrnd)$",
											"VARIANT",
											REG_EXTENDED,
											"Make more localized search in ultraplus model.");
  struct arg_lit *kaffpa_perfectly_balanced_refinement =
	  arg_lit0(NULL, "kaffpa_perfectly_balanced_refinement", "Enable perfectly balanced refinement during ML KaFFPa.");
  struct arg_lit *kaba_disable_zero_weight_cycles =
	  arg_lit0(NULL, "kaba_disable_zero_weight_cycles", "Disable zero weight cycle diversification.");
  struct arg_lit *enforce_balance = arg_lit0(NULL,
											 "enforce_balance",
											 "Uses eps+1 to create a partition, and then runs rebalancing and negative cycle detection to output a partition that fulfills the eps-balance constraint.");
  struct arg_lit *mh_enable_tabu_search = arg_lit0(NULL,
												   "mh_enable_tabu_search",
												   "Enables our version of combine operation by block matching; +tabusearch and all our refinement algorithms.");
  ...
  struct arg_int *cluster_upperbound =
	  arg_int0(NULL, "cluster_upperbound", NULL, "Set a size-constraint on the size of a cluster. Default: none");
  struct arg_int *label_propagation_iterations = arg_int0(NULL,
														  "label_propagation_iterations",
														  NULL,
														  "Set the number of label propgation iterations. Default: 10.");
  struct arg_rex *local_search_neighborhood = arg_rex0(NULL,
													   "local_search_neighborhood",
													   "^(nsquare|nsquarepruned|communication|communicationfaster)$",
													   "VARIANT",
													   REG_EXTENDED,
													   "Local search neighborhood to use (nsquare|nsquarepruned|communication). Default: communication");
  struct arg_int *communication_neighborhood_dist = arg_int0(NULL,
															 "communication_neighborhood_dist",
															 NULL,
															 "set the communication neighborhood distance. Default: 10.");
  struct arg_lit *active_pairs = arg_lit0(NULL, "active_pairs", "Use the active pairs approach.");
  struct arg_rex *construction_algorithm = arg_rex0(NULL,
													"construction_algorithm",
													"^(random|identity|growing|hierarchybottomup|hierarchytopdown)$",
													"VARIANT",
													REG_EXTENDED,
													"Initial construction algorithm to use. One of random, identity, growing, hierarchybottomup, hierarchytopdown. Default: hierarchytopdown.");
  struct arg_rex *distance_construction_algorithm = arg_rex0(NULL,
															 "distance_construction_algorithm",
															 "^(hierarchy|hierarchyonline)$",
															 "VARIANT",
															 REG_EXTENDED,
															 "Construction algorithm to use to initially construct the distance matrix. One of hierarchy, hierarchyonline (does not store distance matrix). Default: hierarchy.");
  struct arg_str *hierarchy_parameter_string = arg_str1(NULL,
														"hierarchy_parameter_string",
														NULL,
														"Specify hierarchy as 2:2:... for 2 cores per PE, 2 PEs per node, and so forth.");
  struct arg_str *distance_parameter_string = arg_str1(NULL,
													   "distance_parameter_string",
													   NULL,
													   "Specify distances between different levels as 1:10:... for 2 cores on the same PE have distance 1, and so forth");
  struct arg_int *search_space_s =
	  arg_int0(NULL, "search_space_s", NULL, "Prune the search space with the paramter s. Default: 64");
  struct arg_int *max_recursion_levels_construction = arg_int0(NULL,
															   "max_recursion_levels_construction",
															   NULL,
															   "Max recursion levels during construction. Default: infinity");
  struct arg_lit *slow_mapping = arg_lit0(NULL,
										  "slow_mapping",
										  "Use the slow computation of gain values (both communication and distances are matricies).");
  struct arg_lit
	  *enable_cycle_local_search = arg_lit0(NULL, "enable_cycle_local_search", "Use local search on cycles.");
  struct arg_lit
	  *enable_group_local_search = arg_lit0(NULL, "enable_group_local_search", "Use local search in groups.");
...
//search_space_s,
	  //slow_mapping,
	  //max_recursion_levels_construction,
	  //enable_cycle_local_search,
	  //enable_group_local_search,
#elif defined MODE_GENERATE_MODEL
	  k, imbalance,
	  user_seed,
	  preconfiguration,
	  filename_output,
...
balance_edges,
  ...
  if (preconfiguration_mapping->count > 0) {
	if (strcmp("strong", preconfiguration_mapping->sval[0]) == 0) {
	  partition_config.preconfiguration_mapping = PRE_CONFIG_MAPPING_STRONG;
	} else if (strcmp("eco", preconfiguration_mapping->sval[0]) == 0) {
	  partition_config.preconfiguration_mapping = PRE_CONFIG_MAPPING_ECO;
	} else if (strcmp("fast", preconfiguration_mapping->sval[0]) == 0) {
	  partition_config.preconfiguration_mapping = PRE_CONFIG_MAPPING_FAST;
	} else {
	  fprintf(stderr, "Invalid preconfiguration mapping variant: \"%s\"\n", preconfiguration_mapping->sval[0]);
	  exit(0);
	}
  }
  ...
  if (mapping_filename->count > 0) {
	partition_config.mapping_filename = mapping_filename->sval[0];
  }
  ...
  if (active_pairs->count) {
	partition_config.active_pairs = true;
  }

  if (slow_mapping->count) {
	partition_config.slow_mapping = true;
  }

  if (local_search_neighborhood->count) {
	if (strcmp("nsquare", local_search_neighborhood->sval[0]) == 0) {
	  partition_config.ls_neighborhood = NSQUARE;
	} else if (strcmp("nsquarepruned", local_search_neighborhood->sval[0]) == 0) {
	  partition_config.ls_neighborhood = NSQUAREPRUNED;
	} else if (strcmp("communication", local_search_neighborhood->sval[0]) == 0) {
	  partition_config.ls_neighborhood = COMMUNICATIONGRAPH;
	} else {
	  fprintf(stderr, "Invalid local search neighborhood variant: \"%s\"\n", local_search_neighborhood->sval[0]);
	  exit(0);
	}
  }

  if (search_space_s->count > 0) {
	partition_config.search_space_s = search_space_s->ival[0];
  }

  if (max_recursion_levels_construction->count > 0) {
	partition_config.max_recursion_levels_construction = max_recursion_levels_construction->ival[0];
  }

  if (communication_neighborhood_dist->count > 0) {
	partition_config.communication_neighborhood_dist = communication_neighborhood_dist->ival[0];
  }

  if (distance_construction_algorithm->count) {
	if (strcmp("random", distance_construction_algorithm->sval[0]) == 0) {
	  partition_config.distance_construction_algorithm = DIST_CONST_RANDOM;
	} else if (strcmp("identity", distance_construction_algorithm->sval[0]) == 0) {
	  partition_config.distance_construction_algorithm = DIST_CONST_IDENTITY;
	} else if (strcmp("hierarchy", distance_construction_algorithm->sval[0]) == 0) {
	  partition_config.distance_construction_algorithm = DIST_CONST_HIERARCHY;
	} else if (strcmp("hierarchyonline", distance_construction_algorithm->sval[0]) == 0) {
	  partition_config.distance_construction_algorithm = DIST_CONST_HIERARCHY_ONLINE;
	} else {
	  fprintf(stderr,
			  "Invalid distance construction algorithm variant: \"%s\"\n",
			  distance_construction_algorithm->sval[0]);
	  exit(0);
	}
  }

  if (construction_algorithm->count) {
	if (strcmp("random", construction_algorithm->sval[0]) == 0) {
	  partition_config.construction_algorithm = MAP_CONST_RANDOM;
	} else if (strcmp("identity", construction_algorithm->sval[0]) == 0) {
	  partition_config.construction_algorithm = MAP_CONST_IDENTITY;
	} else if (strcmp("growing", construction_algorithm->sval[0]) == 0) {
	  partition_config.construction_algorithm = MAP_CONST_OLDGROWING_FASTER;
	} else if (strcmp("hierarchybottomup", construction_algorithm->sval[0]) == 0) {
	  partition_config.construction_algorithm = MAP_CONST_FASTHIERARCHY_BOTTOMUP;
	} else if (strcmp("hierarchytopdown", construction_algorithm->sval[0]) == 0) {
	  partition_config.construction_algorithm = MAP_CONST_FASTHIERARCHY_TOPDOWN;
	} else {
	  fprintf(stderr, "Invalid construction algorithm variant: \"%s\"\n", construction_algorithm->sval[0]);
	  exit(0);
	}
  }
  ...
  if (hierarchy_parameter_string->count) {
	...
	PRINT(std::cout << "new k is " << partition_config.k << std::endl;)
  }
  ...
  if (enable_cycle_local_search->count > 0) {
	partition_config.enable_cycle_local_search = true;
  }

  if (enable_group_local_search->count > 0) {
	partition_config.enable_group_local_search = true;
  }
  ...
  if (permutation_during_refinement->count > 0) {
	if (strcmp("none", permutation_during_refinement->sval[0]) == 0) {
	  ...
	  fprintf(stderr,
			  "Invalid permutation quality during refinement variant: \"%s\"\n",
